...
// Generate batch ID
function generateBatchId(transactions, timestamp) {
    const data = {
...
export function validateTransactionBatch(batch) {
    ...
    if (!batch.transactions || !Array.isArray(batch.transactions))
        return false;
    if (batch.transactions.length === 0 || batch.transactions.length > 10)
        return false;
    if (!batch.batchId || typeof batch.batchId !== 'string')
        return false;
    if (!batch.userPubKey || typeof batch.userPubKey !== 'string')
        return false;
    if (!batch.batchSignature || typeof batch.batchSignature !== 'string')
        return false;
    ...
    if (!batch.transactions.every(tx => tx.userPubKey === userPubKey))
        return false;
    if (batch.userPubKey !== userPubKey)
        return false;
    // Validate batch signature format
    const signatureRegex = /^[a-fA-F0-9]{128}$/;
    if (!signatureRegex.test(batch.batchSignature))
        return false;
    // Validate batch ID format (32 hex chars)
    const batchIdRegex = /^[a-fA-F0-9]{32}$/;
    if (!batchIdRegex.test(batch.batchId))
        return false;
    return true;
}
