...
export class Chain {
  ...
  private async loadFromStorage(): Promise<void> {
    ...
    try {
      ...
      const blocksResult = await new Promise<NewBlock[]>((resolve, reject) => {
      ...
      const leaderboardResult = await new Promise<LeaderboardEntry[]>((resolve, reject) => {
   ...
   */
  private async persistBlock(block: NewBlock): Promise<void> {
   ...
   */
  private async cacheLeaderboard(leaderboard: LeaderboardEntry[]): Promise<void> {
    if (!this.db) return;
   ...
   */
  private validateChainContinuity(block: NewBlock): boolean {
   ...
   */
  private getLastActivity(pubKey: string, chain: NewBlock[]): number {
    let lastActivity = 0;
   ...
   */
  private calculateConsistencyFactor(stats: UserStats): number {
   ...
   */
  private calculateReputationScore(stats: UserStats): number {
   ...
   */
  private calculateLeaderboard(chain: NewBlock[]): LeaderboardEntry[] {
   ...
   */
  async appendBlock(block: NewBlock): Promise<boolean> {
    if (!this.initialized) {
   ...
   */
  async getUserStats(pubKey: string): Promise<LeaderboardEntry | null> {
  ...
  getLatestBlock(): NewBlock | null {
    if (this.storage.blocks.length === 0) return null;
